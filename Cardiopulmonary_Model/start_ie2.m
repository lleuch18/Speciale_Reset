...
while ib<=n
    if mod(ib,10000) == 0 | ib == length(flow)
    disp(['ib ',num2str(ib),' out of ',num2str(n)])
    end
...
while ib==0 && size(brthi,1)>1% ic<=numel(brthi)
    if mod(ib,size(brthi,1)/100) == 0 
        disp(['ib ',num2str(ib),' out of ',num2str(size(brthi,1))])
    end
    ib=brthi(1+ic,2);
    ic=ic+1;
end
...
while ia<=size(brthe,1)-1 && size(brthi,1)>1
    ...
    if mod(ia,size(brthe,1)/100) == 0 
        disp(['ib ',num2str(ib),' out of ',num2str(size(brthe,1))])
    end
